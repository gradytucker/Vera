...
contract Vera {
    ...
    //encrypted bank account balance and cryptocurrency balance
    mapping (address => bytes) private bankBalance;
    mapping (address => bytes) private cryptoBalance;
    ...
    //adding/modifying bank account information by a verifier with the appropriate privileges 
    function addBankInformation(address veraUser, bytes memory encryptedBankInformation) 
    public onlyVerifiers{
        require(verifierPermissions[msg.sender][22]==true,
                "Only registered bank agents can ammend bank information"
                );
        bankBalance[veraUser] = encryptedBankInformation;
    }

    //adding/modifying crypto account information by a verifier with the appropriate privileges 
    function addCryptoInformation(address veraUser, bytes memory encryptedCryptoInformation) 
    public onlyVerifiers{
        require(verifierPermissions[msg.sender][23]==true,
                "Only registered crypto agents can ammend crypto information"
                );
        cryptoBalance[veraUser] = encryptedCryptoInformation;
    }
    ...
    public onlyVerifiers{
        ...
        "Only registered insurance agents can ammend insurance records"
                );
    ...
    function computeVerification(bytes memory key,
        ...
        if(verifierPermissions[v][22] && bytes(nationality[msg.sender]).length>0){
            string memory intermediateResult = decryptBytesToString(bankBalance[msg.sender], key);
            result = append(result, intermediateResult);
        }
        if(verifierPermissions[v][23] && bytes(title[msg.sender]).length>0){
            string memory intermediateResult = decryptBytesToString(cryptoBalance[msg.sender], key);
            result = append(result, intermediateResult);
        }
    ...
    //this function is a helper function that mirrors the above and computes the keccak256
    //hash of passed data to compute and verify emitted events off-chain
    function computeVerificationPublic(bytes memory key,
                                 address verifierAddress, 
                                 string memory timestamp, 
                                 string memory nonce) public view returns (bytes32) {
        //for ease of use, v = verifierAddress :)
        address v = verifierAddress;
        require(verifierList[v] == true, "must be a valid verifier");
        //the address in different formats (bytes, string)
        ...
        //compute the verification hash and return as off-chain result for verifier checks
